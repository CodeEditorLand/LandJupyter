...
export class CodeWatcher implements ICodeWatcher {
	...
	public setDocument(document: TextDocument) {
		...
		this.disposables.push(
			this.codeLensFactory.updateRequired(
		...
		this.disposables.push(
			workspace.onDidCloseNotebookDocument(
				this.ondidCloseNotebook.bind(this),
			),
		);
	}
	...
	public dispose() {
		...
		if (perfMeasures && perfMeasures.codeLensUpdateCount > 0) {
			sendTelemetryEvent(Telemetry.DocumentWithCodeCells, {
				codeLensUpdateTime:
					perfMeasures.totalCodeLensUpdateTimeInMs /
					perfMeasures.codeLensUpdateCount,
	...
	public async runSelectionOrLine(
		...
		if (
			this.document &&
			activeEditor &&
			urlPath.isEqual(activeEditor.document.uri, this.document.uri)
		) {
			const iw = await this.getActiveInteractiveWindow();
			...
			if (text === undefined || isUri(text)) {
				...
				codeToExecute =
					this.executionHelper.getSelectedTextToExecute(activeEditor);
			} else {
				codeToExecute = text;
			}
			if (!codeToExecute) {
	...
	public async runCell(range: Range): Promise<void> {
		...
		const advance =
			...
			this.configService.getSettings(window.activeTextEditor.document.uri)
			...
			editor
				.edit((editBuilder) => {
					...
					for (
						...
						if (
							cellMatcher.isCell(editor.document.lineAt(i).text)
						) {
							lastCell = false;
							index = i;
							editBuilder.insert(
	...
	public deleteCells() {
		...
		if (startCell.range.start.line > 0) {
			...
			startCharacterNumber =
				editor.document.lineAt(startLineNumber).range.end.character;
	...
	private async moveCellsDirection(directionUp: boolean): Promise<boolean> {
		...
		if (directionUp) {
						...
						if (isEditSuccessful) {
							editor.selection = new Selection(
								aboveCell.range.start.line +
									relativeSelectionRange.start.line,
			...
			if (endCellIndex === cells.length - 1) {
				...
				const aboveCellLineLength =
					currentRange.end.line - currentRange.start.line;
				const diffCellLineLength =
					belowCellLineLength - aboveCellLineLength;
				thenSetSelection = thenExchangeTextLines.then(
					(isEditSuccessful) => {
						if (isEditSuccessful) {
							editor.selection = new Selection(
	...
	private getDefaultCellMarker(resource: Resource): string {
		return (
			this.configService.getSettings(resource).defaultCellMarker ||
	...
	private async runMatchingCell(
		...
		if (currentRunCellLens && this.document) {
			const code = dedentCode(
				this.document.getText(currentRunCellLens.range),
			);
