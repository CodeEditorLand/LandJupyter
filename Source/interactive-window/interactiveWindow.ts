...
export class InteractiveWindow implements IInteractiveWindow {
	...
	public get kernelConnectionMetadata():
		| KernelConnectionMetadata
		| undefined {
	...
	private readonly interactiveWindowDebugger:
		| IInteractiveWindowDebugger
		| undefined;
	...
	constructor(
		...
		workspace.onDidCloseNotebookDocument((notebookDocument) => {
			if (
				notebookDocument.uri.toString() === this.notebookUri.toString()
			) {
	...
	public async notifyConnectionReset() {
		if (!this.notebookDocument) {
			const onNotebookOpen = workspace.onDidOpenNotebookDocument(
				async (notebook) => {
					if (
						notebook.uri.toString() === this.notebookUri.toString()
					) {
	...
	public async ensureInitialized() {
		...
		if (this.controller.controller) {
			...
			logger.info(
				"No controller selected for Interactive Window initialization",
			);
			this.controller.setInfoMessageCell(
	...
	public async showInteractiveEditor(): Promise<NotebookEditor> {
			...
			group.tabs.find((tab) => {
				if (
					(tab.input instanceof TabInputNotebook ||
						tab.input instanceof TabInputInteractiveWindow) &&
		...
		const notebook =
			this.notebookDocument || (await this.openNotebookDocument());
	...
	private useNewDebugMode(): boolean {
		...
		return !!(
			...
			(this.controller?.metadata &&
				!isLocalConnection(this.controller.metadata))
		);
	}
	...
	private async submitCode(
		...
		if (
			this.cellMatcher.isEmptyCell(code) ||
					...
					if (ex instanceof InteractiveCellResultError) {
						...
						notebookCellPromise
							.then((cell) =>
								...
								this.errorHandler
									.getErrorMessageForDisplayInCellOutput(
										ex,
										"execution",
										this.owningResource,
									)
	...
	private async createExecutionPromise(
		...
		try {
			...
			if (isDebug && this.useNewDebugMode()) {
				...
				detachKernel = async () =>
					this.interactiveWindowDebugger?.detach(kernel);
			...
			success = await execution
				.executeCell(cell, iwCellMetadata?.generatedCode?.code)
				.then(
					() => true,
					() => false,
				);
	...
	public async scrollToCell(id: string): Promise<void> {
		...
		const matchingCell = editor.notebook
			.getCells()
	...
	public async hasCell(id: string): Promise<boolean> {
		...
		return (
			...
			notebook
				.getCells()
				.some((cell) => getInteractiveCellMetadata(cell)?.id === id)
		);
	}
	...
	private async addNotebookCell(
		...
		const isMarkdown =
			this.cellMatcher.getCellType(code) === MARKDOWN_LANGUAGE;
		...
		const interactiveWindowCellMarker =
			this.cellMatcher.getFirstMarker(code);
