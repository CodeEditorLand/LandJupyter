 ...
 */
export abstract class DataScienceErrorHandler
	implements IDataScienceErrorHandler
{
	...
	public async handleError(err: Error): Promise<void> {
		...
		if (err instanceof JupyterInstallError) {
			...
			window
				.showErrorMessage(getUserFriendlyErrorMessage(err))
				.then(noop, noop);
		} else if (
	...
	public async getErrorMessageForDisplayInCell(
		...
		return this.getErrorMessageForDisplayInCellImpl(
			error,
			errorContext,
			resource,
			false,
	...
	private async getErrorMessageForDisplayInCellImpl(
		...
		if (error instanceof KernelDeadError) {
			...
			error instanceof PackageNotInstalledWindowsLongPathNotEnabledError
		) {
			const packageName =
			...
			(error instanceof KernelDiedError ||
				error instanceof KernelProcessExitedError) &&
			(error.kernelConnectionMetadata.kind ===
			...
			error instanceof BaseKernelError ||
			error instanceof WrappedKernelError
		) {
		...
		if (isCancellationError(err)) {
			...
			switch (selection) {
				case DataScience.removeRemoteJupyterConnectionButtonText: {
					...
					await this.serverUriStorage
						.remove(err.serverProviderHandle)
						.catch(noop);
...
function getUserFriendlyErrorMessage(
	...
	if (error instanceof JupyterNotebookNotInstalled) {
		...
		// hence add a descriptive prefix (message), that provides more context to the user.
		errorMessageSuffix =
			getErrorMessageFromPythonTraceback(error.stdErr) ||
			error.stdErr ||
			error.message;
	} else {
		// These are generic errors, we have no idea what went wrong,
...
function getIPyKernelMissingErrorMessageForCell(
	...
	if (recommendCreatingAndEnvironment) {
		if (wasIPyKernelInstalAttempted(kernelConnection.interpreter)) {
			...
			messages.push(DataScience.createANewPythonEnvironment());
			messages.push(
				DataScience.OrInstallPackageInstructions(
					ipyKernelName,
					installerCommand,
				),
			);
