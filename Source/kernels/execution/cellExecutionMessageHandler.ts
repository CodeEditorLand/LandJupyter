...
export class CellExecutionMessageHandler implements IDisposable {
	...
	public readonly onErrorHandlingExecuteRequestIOPubMessage =
		this._onErrorHandlingIOPubMessage.event;
	...
	private static readonly outputsOwnedByWidgetModel = new Map<
		string,
		Set<string>
	>();
	/**
	...
	private onKernelAnyMessage(
		...
		if (!this.request && direction === "recv") {
			...
			const messageTypesIndicatingEndOfCellExecution = [
				"kernel_info_reply",
				"execute_input",
				"execute_reply",
			];
			...
			if (
				...
				(gotMessageThatIndicatesEndOfCellExecution || gotANewMessage)
			) {
		...
		if (
			jupyterLab.KernelMessage.isCommMsgMsg(msg) &&
	...
	private createTemporaryTask(executionMustBelongToCurrentCell: boolean) {
		...
		if (existingTask) {
			if (
				...
				(executionMustBelongToCurrentCell &&
					existingTask.cell.index === this.cell.index)
			) {
				return existingTask;
			}
		}
		...
		this.previousResultsToRestore = {
			...(this.cell.executionSummary || {}),
		};
		...
		if (
			this.previousResultsToRestore?.executionOrder &&
			this.temporaryExecution
		) {
			this.temporaryExecution.executionOrder =
				this.previousResultsToRestore.executionOrder;
		}
	...
	private handleCommMsg(msg: KernelMessage.ICommMsgMsg) {
		...
		if ("msg_id" in data.state && typeof data.state.msg_id === "string") {
			...
			if (
				...
				(parentHeader &&
					this.ownedRequestMsgIds.has(parentHeader.msg_id))
			) {
	...
	private updateJupyterOutputWidgetWithOutput(
		...
		const commId =
			"commId" in options
				? options.commId
				: outputAreSpecificToAWidgetHandlingCommId;
		if (!commId) {
			return;
		}
		const outputToAppend =
			"outputToAppend" in options ? options.outputToAppend : undefined;

		const expectedModelId =
			this.commIdsMappedToParentWidgetModel.get(commId) || commId;
		...
		const clearOption =
			"clearOutput" in options ? options.clearOutput : false;
		...
		const newOutput = outputToAppend
			? outputsUptoWidget
					.concat(outputToAppend)
					.concat(outputsAfterWidget)
	...
	private async handleInputRequest(msg: KernelMessage.IStdinMessage) {
		...
		if (msg.content && "prompt" in msg.content) {
			...
			const hasPassword =
				msg.content.password !== null &&
				(msg.content.password as boolean);
			await window
				.showInputBox(
					{
						prompt: msg.content.prompt
							? msg.content.prompt.toString()
							: "",
						ignoreFocusOut: true,
	...
	private handleSetNextInput(payload: ISetNextInputPayload) {
		...
		if (payload.replace) {
			...
			cellData.outputs = [];
			cellData.metadata = {};
			const nbEdit = NotebookEdit.insertCells(this.cell.index + 1, [
				cellData,
			]);
	 ...
	 */
	private getOrCreateExecutionTask(
		executionMustBelongToCurrentCell: boolean,
	) {
	...
	private handleStreamMessage(msg: KernelMessage.IStreamMsg) {
		...
		if (
			msg.parent_header.msg_type === "comm_msg" &&
		...
		traceCellMessage(
			this.cell,
			`Update streamed output, new output '${msg.content.text.substring(0, 100)}'`,
		);
		const task = this.getOrCreateExecutionTask(true);
		...
		const { previousValueOfClearOutputOnNextUpdateToOutput } =
			this.clearOutputIfNecessary(task);
	...
	private handleUpdateDisplayDataMessage(
		...
		if (
			outputToBeUpdated.outputItems.length === 0 &&
			newOutput.items.length === 0
		) {
			logger.trace(
