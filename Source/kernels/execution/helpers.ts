...
import {
	base64ToUint8Array,
	uint8ArrayToBase64,
} from "../../platform/common/utils/string";
...
export class NotebookCellStateTracker {
	...
	public static getCellStatus(cell: NotebookCell): string {
		return (
			(
				NotebookCellStateTracker.cellStates.get(cell)
					?.stateTransition || []
			).join(", ") || ""
		);
	}
...
export function traceCellMessage(
	cell: NotebookCell,
...
export async function updateNotebookMetadataWithSelectedKernel(
	...
	if (kernelConnection?.kind === "startUsingPythonInterpreter") {
		...
		let displayName =
			...
			getCachedEnvironment(kernelConnection.interpreter)?.environment
				?.name ||
			"";
		...
		metadata.kernelspec = {
			name:
				originalNameFromOriginalSpecFile ||
		...
		const displayName =
			...
			"";
		const language =
			kernelSpecOrModel.language || kernelSpecOrModel.language || "";
		if (
			metadata.kernelspec.name !== name ||
...
export function findErrorLocation(traceback: string[], cell: NotebookCell) {
	const cellRegex =
		/Cell\s+In\s*\[(?<executionCount>\d+)\],\s*line (?<lineNumber>\d+).*/;
	// older versions of IPython ~8.3.0
	const inputRegex =
		/Input\s+?In\s*\[(?<executionCount>\d+)\][^<]*<cell line:\s?(?<lineNumber>\d+)>.*/;
	let lineNumber: number | undefined = undefined;
	for (const line of traceback) {
		const cleanLine = line.replace(/(?:\u001b\[.+?m)/g, "");
		const lineMatch =
			cellRegex.exec(cleanLine) ?? inputRegex.exec(cleanLine);
