...
abstract class BaseKernel implements IBaseKernel {
	...
	public async start(options?: IStartOptions): Promise<IKernelSession> {
		...
		if (!options?.ignoreTriggeringOnPostInitialized) {
			await this.triggerOnDidStartKernel(
	...
	public async interrupt(): Promise<void> {
		const pendingExecutions = Promise.all(
			Array.from(this.hooks.get("willInterrupt") || new Set<Hook>()).map(
				(h) => h(),
			),
		);
		logger.info(
		...
		try {
			...
			await Promise.all(
				Array.from(
					this.hooks.get("interruptCompleted") || new Set<Hook>(),
				).map((h) => h()),
			).catch(noop);
		}

		logger.info(
			`Interrupt requested & sent for ${getDisplayPath(this.uri)} in notebookEditor.`,
		);
		if (result === InterruptResult.TimedOut) {
			...
			const v = await window.showInformationMessage(
				message,
				{ modal: true },
				yes,
			);
	...
	public async dispose(): Promise<void> {
		...
		const disposeImpl = async () => {
			...
			promises.push(
				Promise.all(
					Array.from(
						this.hooks.get("willCancel") || new Set<Hook>(),
					).map((h) => h()),
				)
					.then(noop)
					.catch(noop),
	...
	public async restart(): Promise<void> {
		try {
			...
			logger.error(
				`Failed to restart kernel ${getDisplayPath(this.uri)}`,
				ex,
			);
			throw ex;
		} finally {
			Promise.all(
				Array.from(
					this.hooks.get("restartCompleted") || new Set<Hook>(),
				).map((h) => h()),
			).catch(noop);
		}
	}
	...
	private async triggerOnDidStartKernel(
		disableUI: boolean,
		isRestarting = false,
	) {
		if (disableUI) {
		...
		if (!this._postInitializedOnStartPromise || isRestarting) {
			...
			this._postInitializedOnStartPromise =
				this._onPostInitialized.fireAsync(
					{},
					this.startCancellation.token,
				);
			void this._postInitializedOnStartPromise.finally(
				() => (this._isPostInitializedInProgress = false),
			);
		}
	...
	private async interruptExecution(
		...
		disposables.push(
			new Disposable(() =>
				swallowExceptions(() =>
					session.statusChanged.disconnect(statusChangedHandler),
				),
			),
		);
		...
		const promise = (async () => {
			...
			const timedOutPromiseDueToDeadKernel =
				createDeferred<InterruptResult>();
			if (this.status === "busy") {
				this.onDisposed(
					() =>
						timedOutPromiseDueToDeadKernel.resolve(
							InterruptResult.Dead,
						),
					this,
					disposables,
				);
	...
	private async initializeAfterStart(session: IKernelSession | undefined) {
		const nb = workspace.notebookDocuments.find(
			(nb) => nb.uri.toString() === this.uri.toString(),
		);
		const tracker = getNotebookTelemetryTracker(nb);
		const postInitialization = nb
			? tracker?.postKernelStartup()
			: undefined;
		try {
			await Promise.all(
				Array.from(this.hooks.get("didStart") || new Set<Hook>()).map(
			...
			if (!this.hookedSessionForEvents.has(session)) {
				...
				this.disposables.push(
					new Disposable(() =>
						swallowExceptions(() =>
							session.statusChanged.disconnect(
								statusChangeHandler,
							),
						),
					),
			...
			session.kernel?.registerCommTarget(
				Identifiers.DefaultCommTarget,
				noop,
			);

			if (
				this.kernelConnectionMetadata.kind ===
				...
				this.gatherInternalStartupCode()
					.then((startupCode) =>
						this.executeSilently(session, startupCode, {
							traceErrors: true,
							traceErrorsMessage:
								"Error executing jupyter extension internal startup code",
						}),
					)
					.catch((ex) =>
						logger.error(
							`Failed to execute internal startup code`,
							ex,
						),
					);
			} else {
	...
	private async determineVersionOfIPyWidgets(session: IKernelSession) {
        ...
        `;
			...
			const version = await this.executeSilently(session, [
				codeToDetermineIPyWidgetsVersion,
			]).catch((ex) =>
			...
			if (Array.isArray(version)) {
				const isVersion8 = version.some((output) =>
					(output.text || "")
						?.toString()
						.includes(`${widgetVersionOutPrefix}8.`),
				);
				const isVersion7 = version.some((output) =>
					(output.text || "")
						?.toString()
						.includes(`${widgetVersionOutPrefix}7.`),
				);
				...
				logger.warn(
					"Failed to determine IPyKernel Version",
					...
					if (
						...
						if (!this._ipywidgetsVersion) {
							...
							if (this.session?.kernel) {
								this.session.kernel.anyMessage.disconnect(
									anyMessageHandler,
									this,
								);
							}
	...
	protected getMatplotLibInitializeCode(): string[] {
		...
		if (this.kernelSettings.themeMatplotlibPlots) {
			...
			logger.ci(
				`Initialize config for plots for ${getDisplayPath(this.resourceUri || this.uri)}`,
			);

			const matplotInit = CodeSnippets.MatplotLibInit;

			logger.trace(
				`Initialize matplotlib for ${getDisplayPath(this.resourceUri || this.uri)}`,
			);
...
function getKernelStartupLogMessage(
	kernel: IBaseKernel,
