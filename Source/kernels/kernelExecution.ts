...
export class NotebookKernelExecution implements INotebookKernelExecution {
	...
	private readonly documentExecutions = new WeakMap<
		NotebookDocument,
		CellExecutionQueue
	>();
	private readonly executionFactory: CellExecutionFactory;
	private readonly _onDidReceiveDisplayUpdate =
		new EventEmitter<NotebookCellOutput>();
	public readonly onDidReceiveDisplayUpdate =
		this._onDidReceiveDisplayUpdate.event;
	...
	public async resumeCellExecution(
		...
		logger.trace(
			`Cell ${cell.index} executed ${success ? "successfully" : "with an error"}`,
		);
	}
	public async executeCell(
		...
		traceCellMessage(
			cell,
			`NotebookKernelExecution.executeCell (2), ${getDisplayPath(cell.notebook.uri)}`,
		);
		// If we're restarting, wait for it to finish
		const sessionPromise = this.kernel.restarting.then(() =>
			this.kernel.start(new DisplayOptions(false)),
		);
		...
		try {
			traceCellMessage(
				cell,
				`NotebookKernelExecution.executeCell (4), ${getDisplayPath(cell.notebook.uri)}`,
			);
	...
	public async *executeCode(
		...
		if (
			...
			extensionId === POWER_TOYS_EXTENSION_ID
		) {
			// If we're restarting, wait for it to finish
			...
			const sessionPromise = this.kernel.restarting.then(() =>
				this.kernel.start(new DisplayOptions(true)),
			);
			...
			const sessionPromise = this.kernel.restarting.then(() =>
				this.kernel.start(
					new StartOptions(
						false,
						this.kernel.isPostInitializedInProgress,
					),
				),
			);
		...
		disposables.push(
			result.onRequestAcknowledged(() =>
				events.executionAcknowledged.fire(),
			),
		);
		...
		token.onCancellationRequested(
			() => {
				...
				logger.debug(
					`Code execution cancelled by extension ${extensionId}`,
				);
			},
			this,
			disposables,
	...
	private async onWillRestart(sessionPromise?: Promise<IKernelSession>) {
		...
		const pendingCells = executionQueue
			? executionQueue
					.cancel(true)
					.then(() => executionQueue.waitForCompletion().catch(noop))
			: Promise.resolve();
	...
	private getOrCreateCellExecutionQueue(
		...
		if (
			existingExecutionQueue &&
			!existingExecutionQueue.isEmpty &&
			!existingExecutionQueue.failed
		) {
