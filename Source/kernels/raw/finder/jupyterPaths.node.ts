...
const winJupyterRuntimePath = path.join(
	"AppData",
	"Roaming",
	"jupyter",
	"runtime",
...
const CACHE_KEY_FOR_JUPYTER_KERNELSPEC_ROOT_PATH =
	"CACHE_KEY_FOR_JUPYTER_KERNELSPEC_ROOT_PATH.";
export const CACHE_KEY_FOR_JUPYTER_KERNEL_PATHS =
...
export class JupyterPaths {
	...
	public async getKernelSpecTempRegistrationFolder() {
		const dir = uriPath.joinPath(
			await getExtensionTempDir(this.context),
			"jupyter",
			"kernels",
		);
	...
	public async getRuntimeDir(): Promise<Uri> {
		...
		const extensionRuntimeDir = Uri.joinPath(
			await getExtensionTempDir(this.context),
			"jupyter",
			"runtime",
		);
	...
	private async getRuntimeDirImpl(): Promise<Uri | undefined> {
		...
		if (process.env["JUPYTER_RUNTIME_DIR"]) {
			runtimeDir = Uri.file(
				path.normalize(process.env["JUPYTER_RUNTIME_DIR"]),
			);
		} else if (userHomeDir) {
			if (this.platformService.isWindows) {
				// On windows the path is not correct if we combine those variables.
		...
		try {
			...
			logger.error(
				`Failed to create/or verify write access to runtime directory ${runtimeDir}`,
				ex,
			);
		}
	}
	...
	public async getDataDirs(options: {
		...
		const key = options.interpreter
			? options.interpreter.uri.toString()
			: "";
	...
	private async getDataDirsImpl(
		...
		if (interpreter) {
			try {
				logger.ci(
					`Getting Jupyter Data Dir for ${interpreter.uri.fsPath}`,
				);
				const factory =
				...
				if (result.stdout.trim().length) {
					...
					logger.debug(
						`Got an empty Jupyter Data Dir from ${interpreter.id}, stderr = ${result.stderr}`,
					);
				}
			} catch (ex) {
				logger.error(
		...
		const jupyterPreferEnvPath = (
			process.env.JUPYTER_PREFER_ENV_PATH || "no"
		).toLowerCase();
	...
	private getSystemJupyterPaths() {
		if (this.platformService.isWindows) {
			const programData = process.env["PROGRAMDATA"]
				? path.normalize(process.env["PROGRAMDATA"])
				: undefined;
			return programData
				? [Uri.joinPath(Uri.file(programData), "jupyter")]
				: [];
		} else {
			return [
				Uri.file("/usr/local/share/jupyter"),
				Uri.file("/usr/share/jupyter"),
			];
		}
	}
	...
	public async getKernelSpecRootPaths(
		...
		if (
			...
			this.cachedKernelSpecRootPaths.stopWatch.elapsedTime <= 60_000
		) {
	...
	private async getJupyterPathSubPaths(
		...
		logger.debug(
			`Jupyter Paths ${getDisplayPath(subDir)}: ${Array.from(paths).map((uri) => getDisplayPath(uri))}`,
		);
		return Array.from(paths);
	}
	...
	private getCachedPaths(): Uri[] {
		return this.globalState
			.get<string[]>(CACHE_KEY_FOR_JUPYTER_KERNEL_PATHS, [])
			.map((s) => Uri.parse(s));
	}
	...
	private updateCachedRootPath(path: Uri | undefined) {
		if (path) {
			...
			this.globalState
				.update(CACHE_KEY_FOR_JUPYTER_KERNELSPEC_ROOT_PATH, undefined)
				.then(noop, noop);
		}
	}
}
