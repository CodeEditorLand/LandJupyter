...
export type KernelSpecFileWithContainingInterpreter = {
...
export class LocalKernelSpecFinder implements IDisposable {
	...
	public async loadKernelSpec(
		...
		if (!this.pathToKernelSpec.has(key)) {
			const promise = this.loadKernelSpecImpl(
				specPath,
				cancelToken,
				interpreter,
			).then(async (kernelSpec) => {
				const globalSpecRootPath =
					await this.jupyterPaths.getKernelSpecRootPath();
			...
			promise
				.finally(() => {
					if (
						...
						this.pathToKernelSpec.get(key) === promise
					) {
						this.pathToKernelSpec.delete(key);
					}
				})
	...
	private async deleteOldKernelSpec(kernelSpecFile: string) {
		...
		const destinationFile = path.join(
			...
			path.basename(kernelSpecFile),
		);
		await this.fs.createDirectory(Uri.file(path.dirname(destinationFile)));
		await this.fs
			.copy(Uri.file(kernelSpecFile), Uri.file(destinationFile))
			.catch(noop);
		await this.fs.delete(Uri.file(kernelSpecFile));
		logger.trace(
			`Old KernelSpec '${kernelSpecFile}' deleted and backup stored in ${destinationFolder}`,
		);
	}
	/**
	...
	public async findKernelSpecsInPaths(
		kernelSearchPath: Uri,
		cancelToken: CancellationToken,
	): Promise<Uri[]> {
		...
		promise
			.finally(() => {
				if (
					cancelToken.isCancellationRequested &&
					this.findKernelSpecsInPathCache.get(cacheKey) === promise
				) {
					this.findKernelSpecsInPathCache.delete(cacheKey);
				}
				disposable.dispose();
		...
		promise.catch((ex) => {
			...
			logger.warn(
				`Failed to search for kernels in ${getDisplayPath(kernelSearchPath)} with an error`,
				ex,
			);
		});
		return promise;
	...
	implements IDisposable, ILocalKernelFinder<T>
{
	...
	constructor(
		...
		this.kernelSpecFinder = new LocalKernelSpecFinder(
			fs,
			memento,
			jupyterPaths,
		);
		...
		this.disposables.push(
			new Disposable(() => dispose(Array.from(this.timeouts.values()))),
		);
	}
	public clearCache() {
	...
	protected async listKernelsFirstTimeFromMemento(
		cacheKey: string,
	): Promise<T[]> {
		const promise = (async () => {
			// Check memento too
			...
			if (
				cache &&
				Array.isArray(cache.kernels) &&
				cache.extensionVersion === this.env.extensionVersion
			) {
				kernels = cache.kernels.map((item) =>
	...
	protected writeToMementoCache(values: T[], cacheKey: string) {
		...
		this.timeouts.set(cacheKey, {
			dispose: () => once(clearTimeout)(timer),
		});
	}
	protected async isValidCachedKernel(
...
export async function loadKernelSpec(
	...
	kernelJson.name =
		kernelJson?.name || path.basename(path.dirname(specPath.fsPath));
	...
	if (
		interpreterPath &&
		!isEmptyCondaEnv &&
		!(await fs.exists(Uri.file(interpreterPath)))
	) {
		return;
	}
