...
import { ignoreLogging, logger } from "../../../platform/logging";
...
} from "../finder/pythonKernelInterruptDaemon.node";
...
const kernelOutputWithConnectionFile =
...
export class TcpPortUsage {
	public static async waitUntilFree(
		port: number,
		retryTimeMs: number,
		timeOutMs: number,
	): Promise<void> {
		...
		try {
			...
			const portsUsed = doNotWaitForZmqPortsToGetUsed
				...
				: Promise.all([
						...
						logger.warn(
							`Waited ${stopwatch.elapsedTime}ms for kernel to start`,
							ex,
						);
			...
			if (
				...
				if (
					...
					e instanceof Error &&
					stdErrToLog.length &&
					e.message.includes(stdErrToLog)
				) {
			...
			if (this.platform.isWindows) {
				...
				await new Promise<void>((resolve) => {
					pidtree(pid, (ex: unknown, pids: number[]) => {
						if (ex) {
							logger.warn(
								`Failed to interrupt children for ${pid}`,
								ex,
							);
						} else {
...
function stripUnwantedMessages(output: string) {
	...
	if (
		...
		lines.some((line) =>
			kernelOutputToNotLog.some((item) => line.includes(item)),
		)
	) {
