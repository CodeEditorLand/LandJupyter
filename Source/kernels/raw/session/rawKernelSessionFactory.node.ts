...
export async function computeLocalWorkingDirectory(
	...
	let suggestedDir = await doComputeLocalWorkingDirectory(
		resource,
		configService,
		fs,
	);
		...
		const workingDir =
			suggestedDir && suggestedDir.includes("${")
				...
				: suggestedDir
					...
					: undefined;
		const expandedWorkingDir = expandWorkingDir(
			workingDir,
			resource,
...
async function doComputeLocalWorkingDirectory(
	...
	if (fileRootStr && vscode.workspace.workspaceFolders?.length) {
		...
		if (path.isAbsolute(fileRootStr)) {
			...
			const combinedPath = vscode.Uri.joinPath(
				workspaceFolderPath,
				fileRootStr,
			);
