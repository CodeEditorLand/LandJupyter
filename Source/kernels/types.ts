...
export interface IBaseKernel extends IAsyncDisposable {
	...
	readonly onPostInitialized: Event<{
 ...
 */
export interface IBaseKernelProvider<T extends IBaseKernel>
 ...
 */
export interface IBaseKernelSession<
...
function sendKernelTelemetry(kernel: KernelConnectionMetadata) {
	...
	if (
		kernelSpec &&
		Array.isArray(kernelSpec.argv) &&
		kernelSpec.argv.length > 0
	) {
		argv0 = kernelSpec.argv[0];
		// eslint-disable-next-line local-rules/dont-use-fspath
		isArgv0SameAsInterpreter =
			argv0.toLowerCase() === interpreter?.uri?.fsPath?.toLowerCase();
