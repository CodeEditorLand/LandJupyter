...
import {
	...
	serializeDataViews,
} from "../../../../platform/common/utils/serializers";
...
export function removeCommTargetFor3rdPartyExtensions(
	kernel: Kernel.IKernelConnection,
	targetName: string,
) {
...
export class IPyWidgetMessageDispatcher implements IIPyWidgetMessageDispatcher {
	...
	private messageHooks = new Map<
		string,
		(msg: KernelMessage.IIOPubMessage) => boolean | PromiseLike<boolean>
	>();
	...
	protected raisePostMessage<
		M extends IInteractiveWindowMapping,
		T extends keyof IInteractiveWindowMapping,
	>(message: IPyWidgetMessages, payload: M[T]) {
	...
	private mirrorExecuteRequest(msg: KernelMessage.IExecuteRequestMsg) {
		...
		this.raisePostMessage(IPyWidgetMessages.IPyWidgets_mirror_execute, {
	...
	private async onKernelSocketMessage(
		protocol: string | undefined,
		data: WebSocketData,
	): Promise<void> {
		...
		if (typeof data === "string") {
			if (shouldMessageBeMirroredWithRenderer(data)) {
				this.raisePostMessage(IPyWidgetMessages.IPyWidgets_msg, {
					id: msgUuid,
					data,
				});
				if (data.includes("display_data")) {
					deserializedMessage = this.deserialize(
						data as any,
						protocol,
					);
					const jupyterLab =
					...
					if (
						jupyterLab.KernelMessage.isDisplayDataMsg(
							deserializedMessage,
						)
					) {
		...
		if (mustDeserialize) {
			const message =
				deserializedMessage ||
				(this.deserialize(data as any, protocol) as any);
	...
	private sendPendingMessages(protocol: string = "") {
		...
		if (!protocol) {
			protocol =
				KernelSocketMap.get(this.kernel.session.kernel.id)?.protocol ||
				"";
		}
		while (this.pendingMessages.length) {
			try {
				...
				const msg: KernelMessage.IMessage =
					typeof message === "string"
						? JSON.parse(message)
						: this.deserialize(message, protocol);
	...
	private async handleKernelRestarts() {
		if (
			this.disposed ||
			this.commTargetsRegistered.size === 0 ||
			!this.kernel?.session
		) {
			return;
		}
	...
	private registerMessageHook(msgId: string) {
		try {
			...
			this.raisePostMessage(
				IPyWidgetMessages.IPyWidgets_ExtensionOperationHandled,
				{
					id: msgId,
	...
	private possiblyRemoveMessageHook(args: {
		hookMsgId: string;
	...
	private handleMessageHookResponse(args: {
		requestId: string;
		parentId: string;
		msgType: string;
		result: boolean;
