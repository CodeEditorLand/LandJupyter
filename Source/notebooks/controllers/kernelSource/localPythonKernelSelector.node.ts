...
import {
	Commands,
	PythonExtension as PythonExtensionId,
...
export class LocalPythonKernelSelector extends DisposableBase {
	...
	constructor(
		...
		const notebookEnvironment =
			ServiceContainer.instance.get<INotebookPythonEnvironmentService>(
		...
		const addCreationCommand = () => {
			...
			const canUseWorkspaceSpecificEnv =
				(workspace.workspaceFolders || [])?.length > 0;
			const recommendCreatingEnv =
				!this.pythonEnvPicker.recommended && canUseWorkspaceSpecificEnv;
				...
				installPythonCommand = this.pythonEnvPicker.addCommand(
					{
						...
						description:
							DataScience.pleaseReloadVSCodeOncePythonHasBeenInstalled,
					},
					() => {
	...
	> {
		...
		if (!extensions.getExtension(PythonExtensionId)) {
			commands
				.executeCommand(Commands.InstallPythonExtensionViaKernelPicker)
				.then(
					(installed) => {
						if (installed === true) {
	...
	private async createNewEnvironment(): Promise<
		Environment | InputFlowAction | undefined
	> {
		const apiProvider =
			ServiceContainer.instance.get<IPythonApiProvider>(
				IPythonApiProvider,
