...
} from "../../kernels/telemetry/notebookTelemetry";
 ...
 */
export class VSCodeNotebookController
	implements Disposable, IVSCodeNotebookController
{
	...
	private pendingCellAdditions = new WeakMap<
		NotebookDocument,
		Promise<void>
	>();
	...
	private runningCellExecutions = new WeakMap<
		NotebookDocument,
		NotebookCellExecution
	>();
	get id() {
	...
	private readonly associatedDocuments = new WeakMap<
		NotebookDocument,
		Promise<void>
	>();
	...
	constructor(
		...
		trackControllerCreation(
			kernelConnection.id,
			kernelConnection.interpreter?.id,
		);
		disposableRegistry.push(this);
		this.displayData = this.displayDataProvider.getDisplayData(
			this.connection,
		);
		...
		this.controller.supportedLanguages =
			this.languageService.getSupportedLanguages(kernelConnection);
	...
	public async restoreConnection(notebook: NotebookDocument) {
		...
		const kernel = await this.connectToKernel(
			notebook,
			new DisplayOptions(true),
		);
		if (this.kernelConnection.kind === "connectToLiveRemoteKernel") {
			const indicator = new RemoteKernelReconnectBusyIndicator(
				kernel,
				this.controller,
				notebook,
			);
		...
		const lastCellExecutionTracker =
			this.serviceContainer.get<LastCellExecutionTracker>(
				LastCellExecutionTracker,
			);
		const info = await lastCellExecutionTracker.getLastTrackedCellExecution(
			notebook,
			kernel,
		);
		...
		const anyMessageHandler = (_: unknown, msg: IAnyMessageArgs) => {
			...
			if (
				getParentHeaderMsgId(msg.msg as KernelMessage.IMessage) ===
				info.msg_id
			) {
		...
		disposeAnyHandler = new Disposable(() => {
			swallowExceptions(() =>
				kernel.session?.kernel?.anyMessage.disconnect(
					anyMessageHandler,
				),
			);
		});
	...
	public postMessage(
		...
		logger.ci(
			`${ConsoleForegroundColors.Green}Posting message to Notebook UI ${messageType}`,
		);
	 ...
	 */
	public setPendingCellAddition(
		notebook: NotebookDocument,
		promise: Promise<void>,
	): void {
		if (this.viewType !== InteractiveWindowView) {
			throw new Error(
				"setPendingCellAddition only applies to the Interactive Window",
			);
		}
	...
	private updateDisplayData() {
		...
		let description =
			this.connection.kind === "connectToLiveRemoteKernel"
				? ""
	...
	private async handleExecution(
		...
		logger.debug(
			`Handle Execution of Cells ${cells.map((c) => c.index)} for ${getDisplayPath(notebook.uri)}`,
		);
	...
	private async onDidChangeSelectedNotebooks(event: {
		...
		if (
			!isJupyterNotebook(event.notebook) &&
			event.notebook.notebookType !== InteractiveWindowView
		) {
			return;
		}
		if (!workspace.isTrusted) {
	...
	private async updateCellLanguages(notebook: NotebookDocument) {
		...
		const preferredLanguage = isPythonKernel
			? PYTHON_LANGUAGE
		...
		await Promise.all(
			notebook
				...
				.map(async (cell) => {
					if (
						!supportedLanguages.includes(cell.document.languageId)
					) {
						await languages
	...
	private handleInterrupt(notebook: NotebookDocument) {
		logger.debug(
			`VS Code interrupted kernel for ${getDisplayPath(notebook.uri)}`,
		);
		notebook
			.getCells()
			.forEach((cell) =>
	...
	private createCellExecutionIfNecessary(
		...
		if (!currentExecution || currentExecution.cell === cell) {
			...
			currentExecution = CellExecutionCreator.getOrCreate(
				cell,
				controller,
				true,
			);
	...
	private async executeQueuedCells(doc: NotebookDocument) {
		...
		const cellExecs: CellExec[] = (this.cellQueue.get(doc) || []).map(
			(cell) => {
				const exec = this.createCellExecutionIfNecessary(
					cell,
					new KernelController(this.controller),
				);
				return { cell, exec };
			},
		);
		...
		try {
			logger.trace(
				`Connect to Kernel ${getDisplayPath(doc.uri)}. Step 2`,
			);
			kernel = await this.connectToKernel(doc, new DisplayOptions(false));
			logger.trace(
				`Connected to Kernel ${getDisplayPath(doc.uri)}. Step 3`,
			);
			if (kernel.disposing) {
			...
			if (kernel && kernel.controller.id !== controller.id) {
				...
				cellExecs.forEach(
					(cellExec) =>
						(cellExec.exec = this.createCellExecutionIfNecessary(
							cellExec.cell,
							controller,
						)),
				);
			...
			const errorHandler =
				this.serviceContainer.get<IDataScienceErrorHandler>(
					IDataScienceErrorHandler,
				);
			ex = WrappedError.unwrap(ex);
			const isCancelled =
				isCancellationError(ex) || ex instanceof KernelDeadError;
			// If there was a failure connecting or executing the kernel, stick it in this cell
			await endCellAndDisplayErrorsInCell(
				firstCell,
				controller,
				await errorHandler.getErrorMessageForDisplayInCellOutput(
					ex,
					currentContext,
					doc.uri,
				),
		...
		await Promise.all(
			cellExecs.map(async ({ cell }) => {
				try {
					...
					const cellExecTracker =
						getNotebookTelemetryTracker(doc)?.executeCell();
					if (cellExecTracker) {
						disposables.add(
							new Disposable(() => cellExecTracker.stop()),
						);
					}
					...
					if (!Exiting.isExiting) {
						...
						lastCellExecutionTracker.deleteTrackedCellExecution(
							cell,
							kernel,
						);
					}
	...
	private updateKernelInfoInNotebookWhenAvailable(
		kernel: IKernel,
		doc: NotebookDocument,
	) {
		...
		handlerDisposables.push({
			dispose: () => statusChangeDisposable.dispose(),
		});
		handlerDisposables.push({
			dispose: () => kernelDisposedDisposable?.dispose(),
		});
	}
	private async onDidSelectController(document: NotebookDocument) {
		...
		if (
			existingKernel &&
			areKernelConnectionsEqual(
				existingKernel.kernelConnectionMetadata,
			...
			window.visibleNotebookEditors
				...
				.forEach((editor) =>
					this.postMessage(
						{
							message:
								IPyWidgetMessages.IPyWidgets_onKernelChanged,
							payload: undefined,
						},
						editor,
					),
		...
		logger.debug(
			`KernelProvider switched kernel to id = ${newKernel.kernelConnectionMetadata.id}`,
		);
		...
		const trustedKernelPaths =
			this.serviceContainer.get<ITrustedKernelPaths>(ITrustedKernelPaths);
		if (
			...
			trustedKernelPaths.isTrusted(
				Uri.file(this.kernelConnection.kernelSpec.specFile),
			)
		) {
...
async function updateNotebookDocumentMetadata(
	...
	const { changed } = await updateNotebookMetadataWithSelectedKernel(
		metadata,
		kernelConnection,
		kernelInfo,
	);
...
export async function warnWhenUsingOutdatedPython(
	kernelConnection: KernelConnectionMetadata,
) {
		...
		window
			...
			.then((selection) => {
				...
				return openInBrowser(
					"https://aka.ms/jupyterUnSupportedPythonKernelVersions",
				);
			}, noop);
	}
}
