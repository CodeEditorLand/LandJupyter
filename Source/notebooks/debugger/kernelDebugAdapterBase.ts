 ...
 */
export abstract class KernelDebugAdapterBase
	implements DebugAdapter, IKernelDebugAdapter, IDisposable
{
		...
		this.disposables.push(
			notebookCellExecutions.onDidChangeNotebookCellExecutionState(
				(cellStateChange: NotebookCellExecutionStateChangeEvent) => {
					...
					if (
						this.configuration.__cellIndex ===
							cellStateChange.cell.index &&
						cellStateChange.state ===
	...
	protected lookupCellByLongName(sourcePath: string) {
		...
		for (let [file, cell] of this.fileToCell.entries()) {
			if (
				...
				shortNameMatchesLongName(file, sourcePath)
			) {
				return cell;
			}
		}
	...
	protected async sendMessageToJupyterSession(
		...
		if (
			...
			this.jupyterSession.status === "dead" ||
