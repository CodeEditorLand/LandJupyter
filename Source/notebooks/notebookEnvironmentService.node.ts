...
import {
	...
	type IVSCodeNotebookController,
...
@injectable()
export class NotebookPythonEnvironmentService
	extends DisposableBase
	implements INotebookPythonEnvironmentService
{
	private readonly _onDidChangeEnvironment = this._register(
		new EventEmitter<Uri>(),
	);
	public readonly onDidChangeEnvironment = this._onDidChangeEnvironment.event;

	private readonly notebookWithRemoteKernelsToMonitor =
		new WeakSet<NotebookDocument>();
	private readonly notebookPythonEnvironments = new WeakMap<
		NotebookDocument,
	...
	public getPythonEnvironment(uri: Uri): Environment | undefined {
		...
		return notebook
			? this.notebookPythonEnvironments.get(notebook)
			: undefined;
	}
	...
	private monitorRemoteKernelStart() {
		const trackKernel = async (e: IKernel) => {
			...
			try {
				const env = await this.resolveRemotePythonEnvironment(
					e.notebook,
				);
				if (
					this.controllerRegistration.getSelected(e.notebook)
				...
				logger.error(
					`Failed to get Remote Python Env for ${getDisplayPath(e.notebook.uri)}`,
					ex,
				);
			}
		};
	...
	private notifyLocalPythonEnvironment(
		...
		const connection =
			this.kernelProvider.get(notebook)?.kernelConnectionMetadata ||
			controller.connection;
		const interpreter = connection.interpreter;
		if (
			!isPythonKernelConnection(connection) ||
		...
		void this.resolveAndNotifyLocalPythonEnvironment(
			notebook,
			controller,
			interpreter,
		);
	...
	private async resolveAndNotifyLocalPythonEnvironment(
		...
		if (
			this.controllerRegistration.getSelected(notebook)?.controller !==
			controller.controller
		) {
			logger.trace(
				`Python Env for ${getDisplayPath(notebook.uri)} not determined as controller changed`,
			);
			return;
		}
	...
	private async resolveRemotePythonEnvironment(
		notebook: NotebookDocument,
...
`;
		...
		const output = outputs.find(
			(item) => item.output_type === "stream" && item.name === "stdout",
		);
		...
		const env =
			getCachedEnvironment(executable) ||
			(await getInterpreterInfo({ id: executable }));
