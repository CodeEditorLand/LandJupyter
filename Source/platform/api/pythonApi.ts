...
import {
	trackInterpreterDiscovery,
	trackPythonExtensionActivation,
		...
		if (getEnvironmentType(env) === EnvironmentType.Conda) {
			uri =
				getOSType() === OSType.Windows
					? Uri.joinPath(
							env.environment?.folderUri || Uri.file(env.path),
							"python.exe",
						)
					: Uri.joinPath(
							env.environment?.folderUri || Uri.file(env.path),
...
export class OldPythonApiProvider implements IPythonApiProvider {
	...
	public async getNewApi(): Promise<PythonExtensionApi | undefined> {
		await this.init();
		const extension =
			extensions.getExtension<PythonExtensionApi>(PythonExtension);
		if (extension?.packageJSON?.version) {
			this._pythonExtensionVersion = new SemVer(
...
export class PythonExtensionChecker implements IPythonExtensionChecker {
	...
	private readonly pythonExtensionInstallationStatusChanged =
	...
	public async showPythonExtensionInstallRequiredPrompt(): Promise<void> {
		...
		sendTelemetryEvent(Telemetry.PythonExtensionNotInstalled, undefined, {
			action: "displayed",
		});
	...
	private async installPythonExtension() {
		...
		commands
			.executeCommand("extension.open", PythonExtension)
			.then(noop, noop);
	}
	...
	private async extensionsChangeHandler(): Promise<void> {
		...
		if (newInstallState !== this.previousInstallState) {
			this.pythonExtensionInstallationStatusChanged.fire(
				newInstallState ? "installed" : "uninstalled",
			);
...
export class InterpreterService implements IInterpreterService {
	private readonly didChangeInterpreter = new EventEmitter<
		PythonEnvironment | undefined
	>();
	private readonly didChangeInterpreters = new EventEmitter<
		PythonEnvironment[]
	>();
	private readonly _onDidEnvironmentVariablesChange =
		new EventEmitter<void>();
	private readonly _onDidRemoveInterpreter = new EventEmitter<{
		id: string;
	}>();
	public onDidRemoveInterpreter = this._onDidRemoveInterpreter.event;
	public onDidEnvironmentVariablesChange =
		this._onDidEnvironmentVariablesChange.event;
	...
	constructor(
		...
		this.refreshPromises.onStateChange(() => {
			this.status = this.refreshPromises.isComplete
				? "idle"
				: "refreshing";
		});
	}
	...
	public async hasWorkspaceSpecificEnvironment(): Promise<boolean> {
		...
		if (
			(workspace.workspaceFolders || []).some((folder) =>
				findPythonEnvBelongingToFolder(
					folder.uri,
	...
	public async getActiveInterpreter(
		...
		if (workspace.workspaceFolders?.length) {
			resource =
				(resource
					? workspace.getWorkspaceFolder(resource)?.uri
					: undefined) || workspace.workspaceFolders[0].uri;
		}
		...
		promise
			...
			.catch((ex) => {
				logger.warn(
					`Failed to get active interpreter from Python for workspace ${workspaceId}`,
					ex,
				);
			});
		if (
			isCI ||
			[ExtensionMode.Development, ExtensionMode.Test].includes(
	...
	public async getInterpreterDetails(
		...
		if (!workspace.isTrusted) {
			throw new Error(
				"Unable to determine active Interpreter as Workspace is not trusted",
			);
		}
	...
	private pendingInterpreterChangeEventTriggers = new Map<
		InterpreterId,
		PythonEnvironment | undefined
	>();
	private pendingInterpretersChangeEventTriggers = new Map<
		InterpreterId,
		PythonEnvironment | undefined
	>();
	private triggerEventIfAllowed(
		...
		if (changeType === "interpreterChangeEvent") {
			this.pendingInterpreterChangeEventTriggers.set(
				interpreter?.id || "",
				interpreter,
			);
		} else {
			this.pendingInterpretersChangeEventTriggers.set(
				interpreter?.id || "",
				interpreter,
			);
		}
	...
	private triggerPendingEvents() {
		...
		const interpreters = Array.from(
			this.pendingInterpretersChangeEventTriggers.values(),
		);
		if (interpreters.length) {
			const nonEmptyInterpreterList = interpreters.filter(
				(item) => !!item,
			) as PythonEnvironment[];
			if (
				nonEmptyInterpreterList.length !== interpreters.length &&
				nonEmptyInterpreterList.length === 0
			) {
			...
			.then((api) => {
				if (!this.eventHandlerAdded && api) {
					...
					api.environments.onDidEnvironmentVariablesChange(
						(e) => {
							logger.debug(
								`Detected changes to env file ${e.resource?.uri?.path} in PythonApi`,
							);
					...
					api.environments.onDidChangeEnvironments(
						async (e) => {
							logger.trace(
								`Python API env change detected, ${e.type} => '${e.env.id}'`,
							);
							...
							if (e.type === "remove") {
								...
								this.triggerEventIfAllowed(
									"interpretersChangeEvent",
									undefined,
								);
								this._onDidRemoveInterpreter.fire({
									id: e.env.id,
								});
								return;
							}
							const info = resolvedPythonEnvToJupyterEnv(
								getCachedEnvironment(e.env),
							);
							if (info) {
								this.triggerEventIfAllowed(
									"interpreterChangeEvent",
