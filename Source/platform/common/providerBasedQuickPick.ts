...
interface SelectorQuickPickItem<T extends { id: string }>
...
> extends DisposableBase {
	...
	private createQuickPick() {
		...
		this.provider
			...
			.catch((ex) =>
				logger.error(
					`Failed to get quick pick items for ${quickPick.title}`,
					ex,
				),
			);
		...
		while (!token.isCancellationRequested) {
			...
			try {
				...
				if (
					result &&
					"label" in result &&
					this.isCommandQuickPickItem(result)
				) {
		...
		this.quickPickItems = this.quickPickItems.map((item) => {
			if (this.isSelectorQuickPickItem(item)) {
				...
				if (latestFromProvider) {
					const latestQuickPickItem =
						this.toQuickPickItem(latestFromProvider);
					...
					if (
						...
						JSON.stringify({ ...latestQuickPickItem, item: "" })
					) {
		...
		groupBy(newQuickPickItems, (a, b) =>
			...
			if (existingCategory) {
				const indexOfExistingCategory =
					this.quickPickItems.indexOf(existingCategory);
				const currentItemsInCategory =
					this.categories.get(existingCategory)!;
				...
				const currentCategories: [CategoryQuickPickItem, number][] =
					this.quickPickItems
						.filter((item) => item instanceof CategoryQuickPickItem)
						.map((item, index) => [
							item as CategoryQuickPickItem,
							index,
						]);
	...
	private rebuildQuickPickItems(quickPick: QuickPick<QuickPickItem>) {
		...
		const indexOfSeparator = this.commands.findIndex(
			(c) =>
				...
				c.label === DataScience.recommendedItemCategoryInQuickPick,
		);
		...
		let selectedQuickPickItem:
			| SelectorQuickPickItem<T>
			| CommandQuickPickItem<T>
			| undefined = recommendedItemQuickPick || recommendedCommandItem;
		...
		if (selectedQuickPickItem && currentActiveItem) {
			if (!this.isSelectorQuickPickItem(currentActiveItem)) {
				...
				if (
					...
					recommendedCommandItem?.label === currentActiveItem.label
				) {
					// Do nothing.
				} else {
