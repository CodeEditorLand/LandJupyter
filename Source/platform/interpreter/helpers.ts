...
export function getPythonEnvDisplayName(
	...
	const pythonVersion = getTelemetrySafeVersion(
		getCachedVersion(interpreter) || "",
	).trim();
...
export function getPythonEnvironmentName(pythonEnv: PythonEnvironment) {
	...
	if (
		...
		getEnvironmentType(pythonEnv) === EnvironmentType.Conda
	) {
...
export function getEnvironmentType(interpreter: {
	id: string;
}): EnvironmentType {
	const env = getCachedEnvironment(interpreter);
...
function getEnvironmentTypeImpl(env: Environment): EnvironmentType {
	...
	const orderOrEnvs: [
		pythonEnvTool: KnownEnvironmentTools,
		JupyterEnv: EnvironmentType,
	][] = [
	...
	for (const type of environmentTypes) {
		if (
			env.tools.some((tool) => tool.toLowerCase() === type.toLowerCase())
		) {
			return type;
		}
	}
	...
	if (typeof interpreter === "string") {
		return pythonApi.environments.known.find(
			...
			(i) =>
				i.id === interpreter ||
				i.path === interpreter ||
				i.executable.uri?.fsPath === interpreter,
		);
	}
