...
import { CancellationToken, EventEmitter, Uri, workspace } from "vscode";
...
const extensionAPICache = new Map<
	string,
	{
		...
		kernels: WeakMap<
			IKernel,
			{ api: Kernel; progress: KernelExecutionProgressIndicator }
		>;
	}
>();
...
function getWrappedKernel(kernel: IKernel, extensionId: string) {
	...
	let wrappedKernel =
		extensionAPI.kernels.get(kernel) ||
...
export function getKernelsApi(extensionId: string): Kernels {
	return {
		...
		get onDidStart() {
			if (
				![JVSC_EXTENSION_ID, DATA_WRANGLER_EXTENSION_ID].includes(
					extensionId,
				) &&
				!PROPOSED_API_ALLOWED_PUBLISHERS.includes(
					extensionId.split(".")[0],
				)
			) {
				throw new Error(
					`Proposed API is not supported for extension ${extensionId}`,
				);
			}
			...
			if (!extensionAPI.onDidStart) {
				...
				disposableRegistry.push(
					...
					kernelProvider.onDidPostInitializeKernel(
						({ kernel, token, waitUntil }) => {
							const { api, progress } = getWrappedKernel(
								kernel,
								extensionId,
							);
