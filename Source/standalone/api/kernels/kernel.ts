...
import {
	ChatMime,
	generatePythonCodeToInvokeCallback,
} from "../../../kernels/chat/generator";
import {
...
class WrappedKernelPerExtension extends DisposableBase implements Kernel {
	...
	constructor(
		...
		this._api = Object.freeze({
			...
			executeCode: (code: string, token: CancellationToken) =>
				this.executeCode(code, token),
			executeChatCode: (
				code: string,
				handlers: Record<
					string,
	...
	static createApiKernel(
		...
		const wrapper = new WrappedKernelPerExtension(
			extensionId,
			kernel,
			execution,
			controller,
...
async function sendApiTelemetry(
	extensionId: string,
	kernel: IKernel,
