...
class ProxyKernel implements IMessageHandler, Kernel.IKernelConnection {
	private readonly _ioPubMessageSignal: Signal<
		this,
	...
	constructor(
		...
		class ProxyWebSocket {
			...
			public onmessage?:
				| ((this: ProxyWebSocket, ev: MessageEvent) => any)
				| null;
				...
				if (this.sendEnabled) {
					if (typeof data === "string") {
						postOffice.sendMessage<IInteractiveWindowMapping>(
							IPyWidgetMessages.IPyWidgets_msg,
							data,
						);
					} else {
		...
		const signaling =
			require("@lumino/signaling") as typeof import("@lumino/signaling");
		this._ioPubMessageSignal = new signaling.Signal<
			this,
			KernelMessage.IIOPubMessage
		>(this);
	...
	public requestComplete(content: {
		code: string;
		cursor_pos: number;
	...
	public requestIsComplete(content: {
		code: string;
	...
	public registerMessageHook(
		...
		this.postOffice.sendMessage<IInteractiveWindowMapping>(
			IPyWidgetMessages.IPyWidgets_RegisterMessageHook,
			msgId,
		);
	...
	public removeMessageHook(
		...
		const key = this.generateExtensionResponseKey(
			msgId,
			IPyWidgetMessages.IPyWidgets_RemoveMessageHook.toString(),
		);
		this.awaitingExtensionMessage.set(key, waitPromise);
		...
		this.postOffice.sendMessage<IInteractiveWindowMapping>(
			...
			{
				hookMsgId: msgId,
				lastHookedMsgId: this.lastHookedMessageId.length
					? this.lastHookedMessageId.pop()
					: undefined,
			},
		);
	...
	private sendResponse(id: string) {
		this.postOffice.sendMessage<IInteractiveWindowMapping>(
			IPyWidgetMessages.IPyWidgets_msg_received,
			{
				id,
			},
		);
	...
	private generateExtensionResponseKey(
		msgId: string,
		msgType: string,
	): string {
	...
	private fakeOpenSocket() {
		...
		const originalRequestKernelInfo =
	...
	private handleMirrorExecute(payload: {
		id: string;
			...
			Promise.all(extensionPromises)
				...
				.catch((ex) => {
					window.console.error(
						"Failed to send iopub_msg_handled message",
						ex,
					);
				});
		}
