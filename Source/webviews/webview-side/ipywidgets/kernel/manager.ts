...
export class WidgetManager implements IIPyWidgetManager, IMessageHandler {
	...
	private initializeKernelAndWidgetManager(
		...
		if (
			this.manager &&
			this.proxyKernel &&
			fastDeepEqual(options, this.options)
		) {
			return;
		}
		...
		try {
			...
			renderersAndMimetypes.forEach((mimeTypes, rendererId) => {
				mimeTypes.forEach((mime) => {
					if (
						registeredMimeTypes.has(mime) ||
						!manager.registerMimeRenderer
					) {
						return;
					}
					...
					manager.registerMimeRenderer(
						mime,
						async ({ data, metadata }, node) => {
							// eslint-disable-next-line @typescript-eslint/no-explicit-any
							const context = (globalThis as any)
								.jupyter_vscode_rendererContext as RendererContext<any>;
							const renderer =
								await context.getRenderer(rendererId);
							const isImage =
								mime.toLowerCase().startsWith("image/") &&
								!mime.toLowerCase().includes("svg");
							const renderOutputItem =
								renderer?.renderOutputItem as
									| undefined
									| ((
	 ...
	 */
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	private async handleDisplayDataMessage(
		_sender: any,
